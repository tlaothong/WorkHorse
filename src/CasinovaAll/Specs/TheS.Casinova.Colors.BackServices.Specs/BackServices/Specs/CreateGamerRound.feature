...
@record_mock
Scenario: ได้รับข้อมูล config มีโต๊ะเกมที่เริ่มเล่นอยู่ในระบบไม่ตรงกับค่าที่ตั้งไว้, ระบบสร้างโต๊ะเกมที่เหลือ
...
@record_mock
Scenario: ได้รับข้อมูล config มีโต๊ะเกมที่เริ่มเล่นอยู่ในระบบตรงกับค่าที่ตั้งไว้, ระบบเสมือนทำงานสำเร็จ
	Given The CreateGameRoundsExecutor has been created and initialized
	And sent Name: 'config1', the GameRoundConfiguration should recieved
	And server should recieved the active game round data as:
	|RoundID	|StartTime			|EndTime			|
...
@record_mock
Scenario: ได้รับข้อมูล config ไม่มีโต๊ะเกมที่เริ่มเล่นอยู่ในระบบ, ระบบสร้างโต๊ะเกมใหม่ทั้งหมด
	Given The CreateGameRoundsExecutor has been created and initialized
	And sent Name: 'config1', the GameRoundConfiguration should recieved
	And server should recieved the active game round data as:
	|RoundID	|StartTime			|EndTime			|

	And Expect result should be create at datetime (2553/3/12 10:10):
	|RoundID	|StartTime			|EndTime			|
	|1			|2553/3/12 10:00	|2553/3/12 10:30	|
	|2			|2553/3/12 10:05	|2553/3/12 10:35	|
	|3			|2553/3/12 10:10	|2553/3/12 10:40	|
