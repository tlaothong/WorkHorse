...
Background:
	Given Server has game play information as:
		|UserName|RoundID	|TotalBetAmountOfBlack|TotalBetAmountOfWhite|Winner|WinnerLastUpdate|			TrackingID				   |		OnGoingTrackingID			  | 
...
@record_mock
Scenario: ระบบได้รับข้อมูล username ถูกต้อง และมีข้อมูลการเล่นเกมของผู้เล่น ระบบลิสต์ข้อมูลโต๊ะเกมที่ผู้เล่นเคยเล่นไว้ได้
...
@record_mock
Scenario: ระบบได้รับข้อมูล username ถูกต้อง ระบบลิสต์ข้อมูลโต๊ะเกมแต่ไม่มีข้อมูล เนื่องจากผู้เล่นยังไม่เคยเล่นเกมที่โต๊ะใด ๆ 
	Given The ListGamePlayInfoExecutor has been created and initialized	
