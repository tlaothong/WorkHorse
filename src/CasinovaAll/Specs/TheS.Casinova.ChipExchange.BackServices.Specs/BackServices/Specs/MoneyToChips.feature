...
@record_mock
Scenario: (MoneyToChips)ผู้เล่นแลกเงินเป็นชิฟเป็น บัตรเครดิตของผู้เล่นถูกต้อง ระบุจำนวนเงินมากกว่าจำนวนขั้นต่ำ, ระบบเพิ่มชิฟเป็นให้กับผู้เล่น
	Given The MoneyToChipsExecutor has been created and initialized
	And (MoneyToChips)sent ExchangeSettingName: 'exchange1' the exchange setting should recieved 
	And (MoneyToChips)sent UserName: 'OhAe', AccountType: 'Primary' the player account information should recieved
	And (MoneyToChips)the PayExchangeEngine should be call and complete transaction sent UserName: 'OhAe', Amount: '2000', CardType: 'VISA', FistName: 'Sirinarin', LastName: 'AAA', AccountNo: '123445677891', CVV: '1234', ExpireDate: '2009/12'
	And (MoneyToChips)the user chips should be adding(UserName: 'OhAe', Amount:'2000')
...
@record_mock
Scenario: (MoneyToChips)ผู้เล่นแลกเงินเป็นชิฟเป็น บัตรเครดิตของผู้เล่นถูกต้อง ระบุจำนวนน้อยกว่าจำนวนขั้นต่ำ, ระบบไม่อนุญาติให้แลกเงิน
	Given The MoneyToChipsExecutor has been created and initialized
	And (MoneyToChips)sent ExchangeSettingName: 'exchange1' the exchange setting should recieved 
	And (MoneyToChips)sent UserName: 'OhAe', AccountType: 'Primary' the player account information should recieved
	When Expected exception and call MoneyToChipsExecutor(UserName: 'OhAe', Amount: '500', AccountType: 'Primary')
